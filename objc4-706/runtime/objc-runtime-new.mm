                       ...
                       bool initialize, bool cache, bool resolver)
{
 ...
 retry:
    ...
    // Try this class's cache. 在缓存中查找

    imp = cache_getImp(cls, sel);
    if (imp) goto done;

    // Try this class's method lists. 在类的方法列表中查找
    ...
    // Try superclass caches and method lists. 在父类的缓存和方法列表中查找
    ...
    // No implementation found. Try method resolver once. 没有找到方法实现, 尝试方法解析器
    ...
    runtimeLock.unlockRead(); // 解锁, 返回实现
